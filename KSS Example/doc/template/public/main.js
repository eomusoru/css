(function ($) {
    $(function () {
        ...
        $('.js-anim-toggle').on('click', function(){
            var animName   = $(this).attr('data-class'),
                $animBlock = $(this).prev('.js-anim-target');
                
            $animBlock.removeClass(animName);
            
            setTimeout(function(){
                $animBlock
                    .addClass('anim')
                    .toggleClass(animName);
            }, 150);
        });
        
        $(document).ready(function(){
            $('[data-class*="$modifier"]').closest('.kss-modifier').remove();
        });
    });
}(jQuery));